...
void (function () {
  ...
  function promptYesNo (question, ifYes, ifNo, defaultAnswer) {
    ...
    }, function (err, value) { // eslint-disable-line handle-callback-err
      if (yesno(value)) {
  ...
  function captureCredentials (usernameIn, passwordIn) {
    ...
    function getUsername (callback) {
      promptly.prompt('username: ', function (err, user) { // eslint-disable-line handle-callback-err
        callback(user)
      })
    }

    function getPassword (username, callback) {
      promptly.password('password: ', function (err, pass) { // eslint-disable-line handle-callback-err
  ...
  function toBase64 (input) {
    ...
    return new Buffer(input).toString('base64') // eslint-disable-line
  }
